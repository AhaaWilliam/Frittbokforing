...
test.describe('Kontoplan + Backup', () => {
  test('kontoplan listas via IPC och page-accounts renderar', async () => {
    ...
    try {
      ...
      const r = result as { success: boolean; data: Array<{ account_number: string }> }
      expect(r.success).toBe(true)
      expect(r.data.length).toBeGreaterThan(50)

      await ctx.window.evaluate(() => { location.hash = '#/accounts' })
      await expect(ctx.window.getByTestId('page-accounts')).toBeVisible({ timeout: 10_000 })
    } finally {
      await ctx.cleanup()
    }
  })
  ...
  test('backup via dialog-bypass skapar .db-fil i downloadDir', async () => {
    ...
    try {
      ...
      const result = await ctx.window.evaluate(async () => {
        return (window as unknown as { api: { backupCreate: () => Promise<unknown> } }).api.backupCreate()
      })
      const r = result as { success: boolean; data?: { filePath: string | null }; error?: string }
