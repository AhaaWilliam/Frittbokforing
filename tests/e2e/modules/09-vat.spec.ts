  ...
  test('IPC returnerar utgående moms efter bokförd 25%-faktura + sida renderar', async () => {
    ...
    try {
      ...
      const vatResult = await ctx.window.evaluate(async (fy) => {
        return (window as unknown as { api: { getVatReport: (d: { fiscal_year_id: number }) => Promise<unknown> } }).api.getVatReport({ fiscal_year_id: fy })
      }, fiscalYearId)
      const vat = vatResult as { success: boolean; data: { quarters: Array<Record<string, unknown>> } }
      expect(vat.success).toBe(true)
      expect(vat.data.quarters.length).toBe(4)

      await ctx.window.evaluate(() => { location.hash = '#/vat' })
      await expect(ctx.window.getByTestId('page-vat')).toBeVisible({ timeout: 10_000 })
    } finally {
      await ctx.cleanup()
    }
  })
