...
test.describe('Tvärgående', () => {
  test('K2 och K3 ger olika kontoplan-listor', async () => {
    ...
    try {
      await expect(ctxK2.window.getByTestId('wizard')).toBeVisible({ timeout: 15_000 })
      await composeEmptyK2(ctxK2.window)
      await expect(ctxK2.window.getByTestId('app-ready')).toBeVisible({ timeout: 15_000 })
      ...
      try {
        await expect(ctxK3.window.getByTestId('wizard')).toBeVisible({ timeout: 15_000 })
        await composeEmptyK3(ctxK3.window)
        await expect(ctxK3.window.getByTestId('app-ready')).toBeVisible({ timeout: 15_000 })
  ...
  test('forcePeriodState stänger period via __testApi', async () => {
    ...
    try {
      ...
      const periods = (pRes as { data: Array<{ id: number; is_closed: number }> }).data
      ...
      const updated = (pAfter as { data: Array<{ id: number; is_closed: number }> }).data
