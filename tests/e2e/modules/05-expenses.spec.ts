...
test.describe('Leverantörsfakturor', () => {
  test('skapa + boka → B1 skapas', async () => {
    ...
    try {
      ...
      const b1 = entries.find(
        (e) => e.verification_series === 'B' && e.verification_number === 1,
      )
  ...
  test('B-serie gapless oberoende av A-serie', async () => {
    ...
    try {
      ...
      await createAndFinalizeExpense(ctx.window, {
        supplierId,
        fiscalYearId,
        invoiceDate: '2026-03-15',
        amount_ore: 10000,
      ...
      const { entries } = await getJournalEntries(ctx.window, fiscalYearId)
      const bEntries = entries.filter(
        (e) => e.verification_series === 'B' && e.status === 'booked',
      )
      expect(
        bEntries.map((e) => e.verification_number).sort((a, b) => a - b),
      ).toEqual([1, 2])
    } finally {
      await ctx.cleanup()
    }
