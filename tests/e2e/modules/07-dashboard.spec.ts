  ...
  test('renderar och reflekterar bokförd intäkt via IPC', async () => {
    ...
    try {
      ...
      const beforeResult = await ctx.window.evaluate(async (fy) => {
        ...
        ).api.getDashboardSummary({ fiscalYearId: fy })
      }, fiscalYearId)
      const before = beforeResult as {
        success: boolean
        data: { revenueOre: number }
      }
      ...
      await expect(ctx.window.getByTestId('page-overview')).toBeVisible({
        timeout: 10_000,
      })
    } finally {
      await ctx.cleanup()
