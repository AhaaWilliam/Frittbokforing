...
async function createProduct(
  ...
  opts: { name: string; article_type: 'service' | 'goods' | 'expense'; accountNumber: string },
): Promise<number> {
  ...
  const accounts = await window.evaluate(async () => {
    return (window as unknown as { api: { listAllAccounts: (d: unknown) => Promise<unknown> } }).api.listAllAccounts({})
  })
  const acctList = (accounts as { data: Array<{ id: number; account_number: string }> }).data
  ...
  const result = await window.evaluate(async (data) => {
    return (window as unknown as { api: { createProduct: (d: unknown) => Promise<unknown> } }).api.createProduct(data)
  }, {
    name: opts.name,
    unit: 'styck',
...
test.describe('Artiklar', () => {
  test('skapa tjänst + vara + utlägg och lista dem', async () => {
    ...
    try {
      ...
      await createProduct(ctx.window, { name: 'Konsulttjänst', article_type: 'service', accountNumber: '3002' })
      await createProduct(ctx.window, { name: 'Mus', article_type: 'goods', accountNumber: '3001' })
      await createProduct(ctx.window, { name: 'Frakt', article_type: 'expense', accountNumber: '3004' })

      await ctx.window.evaluate(() => { location.hash = '#/products' })
      await expect(ctx.window.getByTestId('page-products')).toBeVisible({ timeout: 10_000 })
      await expect(ctx.window.getByText('Konsulttjänst')).toBeVisible({ timeout: 5_000 })
