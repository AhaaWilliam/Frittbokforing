...
async function createManual(
  ...
  fiscalYearId: number,
  lines: Array<{ account_number: string; debit_ore: number; credit_ore: number }>,
  entryDate = '2026-03-15',
): Promise<{ success: boolean; data?: { id: number }; error?: string }> {
  const draft = await window.evaluate(async (args) => {
    return (window as unknown as { api: { saveManualEntryDraft: (d: unknown) => Promise<unknown> } }).api.saveManualEntryDraft({
  ...
  const fin = await window.evaluate(async (args) => {
    return (window as unknown as { api: { finalizeManualEntry: (d: { id: number; fiscal_year_id: number }) => Promise<unknown> } }).api.finalizeManualEntry({ id: args.id, fiscal_year_id: args.fy })
...
test.describe('Manuella verifikat', () => {
  test('balanserad C bokförs som C1', async () => {
    ...
    try {
      ...
      const c1 = entries.find(e => e.verification_series === 'C' && e.verification_number === 1)
