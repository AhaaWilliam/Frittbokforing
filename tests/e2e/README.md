...
- Vardag-sheets (VS-3..VS-28): `vardag-bigbtn-kostnad`, `vardag-bigbtn-faktura`, `vardag-bigbtn-stang-manad`, `vardag-hero`, `vardag-shell`, `vardag-kostnad-{date,amount,supplier,description,account,vat,submit,error,receipt-pick,receipt-attached,receipt-clear,multiline-cta,date-error,account-error,account-name,missing-hint}`, `vardag-faktura-{customer,date,payment-terms,description,qty,price,line-total,account,vat,submit,error,multiline-cta,date-error,account-error,account-name,missing-hint}`
- Vardag OCR (VS-145/152): `vardag-kostnad-ocr-{loading,suggestion,supplier-match,supplier-hint,apply,dismiss,create-supplier}` — `create-supplier` är "Skapa '{hint}' som leverantör"-knapp som visas när OCR-hint finns utan fuzzy-match (VS-152).
- Period status (VS-122/127/130): `period-checks-status-{period_id}` — status-pill på första öppna period (VS-122). `period-closed-{period_id}` — span med "Stängd `<datum>`"-text och full timestamp i title-attr (VS-130).
- Year picker (VS-135): `year-picker-lock-label` — "Stängt `<datum>` — skrivskyddat" lock-label på readonly fiscal year. Title-attr bär hela closed_at-timestamp.
