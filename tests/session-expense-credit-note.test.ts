...
function createFinalizedExpense(
  ...
  describe('Omvänd bokföring', () => {
    it('kreditnota bokförs med omvända journal lines (K kostnadskonto, K 2640, D 2440)', () => {
      ...
      const je = db
        ...
        .get(journalEntryId) as { journal_entry_id: number }
    ...
    it('verifikationstext innehåller referens till originalkostnad', () => {
      ...
      const je = db
        .prepare('SELECT journal_entry_id FROM expenses WHERE id = ?')
        .get(cn.data.id) as { journal_entry_id: number }
      const entry = db
