...
function seedBase() {
  createCompany(db, {
    name: 'Test AB', org_number: '556036-0793', fiscal_rule: 'K2',
    share_capital: 2_500_000, registration_date: '2026-01-15',
...
describe('rebuildSearchIndex', () => {
  ...
  it('product searchable after rebuild', () => {
    createProduct(db, {
      name: 'Konsulttimme', unit: 'timme', default_price_ore: 100000,
      vat_code_id: 1, account_id: 1, article_type: 'service',
    })
    rebuildSearchIndex(db)
    const count = db.prepare(
  ...
  it('inactive counterparties excluded', () => {
    ...
    db.prepare('UPDATE counterparties SET is_active = 0 WHERE id = ?').run(cp.data.id)
    rebuildSearchIndex(db)
...
describe('FTS5 fallback', () => {
  it('globalSearch works without search_index table (LIKE fallback)', () => {
    ...
    const result = globalSearch(db, { query: 'FallbackTest', fiscal_year_id: fyId })
    const customers = getData(result).results.filter(r => r.type === 'customer')
...
describe('FTS5 incremental', () => {
  ...
  it('updated counterparty name reflected in search', () => {
    ...
    if (!cp.success) throw new Error('CP failed')
    db.prepare('UPDATE counterparties SET name = ? WHERE id = ?').run('New Name AB', cp.data.id)
    rebuildSearchIndex(db)
