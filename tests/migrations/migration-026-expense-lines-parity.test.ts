...
describe('Migration 026: expense_lines sort_order + created_at parity', () => {
  it('adds sort_order and created_at with correct backfill', () => {
    ...
    const oldCols = (db.prepare('PRAGMA table_info(expense_lines)').all() as { name: string }[]).map(c => c.name)
    ...
    const newCols = (db.prepare('PRAGMA table_info(expense_lines)').all() as { name: string }[]).map(c => c.name)
    ...
    const lines = db.prepare('SELECT id, sort_order, created_at FROM expense_lines ORDER BY id').all() as {
      id: number
      sort_order: number
      created_at: string
    }[]
  ...
  it('NOT NULL constraint prevents inserting without sort_order default', () => {
    ...
    const line = db.prepare('SELECT sort_order, created_at FROM expense_lines WHERE expense_id = 1').get() as {
      sort_order: number
      created_at: string
    }
