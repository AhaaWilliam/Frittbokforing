...
describe('opening_balance write-once kontrakt', () => {
  it('Test 1: draft → booked lyckas med balanserade rader', () => {
    ...
    expect(() => {
      ctx.db.prepare('UPDATE journal_entries SET status = ? WHERE id = ?').run('booked', jeId)
    }).not.toThrow()

    const row = ctx.db.prepare('SELECT status FROM journal_entries WHERE id = ?').get(jeId) as { status: string }
  ...
  it('Kontrasttest: vanlig booked entry kan INTE gå tillbaka till draft (trigger 1)', () => {
    ...
    expect(() => {
      ctx.db.prepare('UPDATE journal_entries SET status = ? WHERE id = ?').run('draft', jeId)
  ...
  it('Test 3a: trigger 6 kastar vid bokning med obalanserade rader', () => {
    ...
    expect(() => {
      ctx.db.prepare('UPDATE journal_entries SET status = ? WHERE id = ?').run('booked', jeId)
    }).toThrow('Verifikationen balanserar inte. Summa debet måste vara lika med summa kredit.')
  })
  ...
  it('Test 3b: trigger 7 kastar vid bokning i stängt räkenskapsår', () => {
    ...
    ctx.db.prepare('UPDATE fiscal_years SET is_closed = 1 WHERE id = ?').run(ctx.seed.fiscalYearId)
