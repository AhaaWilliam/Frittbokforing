...
describe('Migration 031: Immutability-hardening triggers', () => {
  it('trg_immutable_source_type blocks UPDATE on booked entry', () => {
    ...
    expect(() => {
      db.prepare(
        "UPDATE journal_entries SET source_type = 'manual' WHERE id = ?",
      ).run(jeId)
  ...
  it('trg_immutable_source_type allows UPDATE on draft', () => {
    ...
    db.prepare(
      ...
      VALUES (1, ?, 'C', '2026-03-01', 'Test draft', 'draft', 'manual')`,
    ...
    db.prepare(
      "UPDATE journal_entries SET source_type = 'auto_invoice' WHERE id = ?",
    ).run(jeId)
    const row = db
      .prepare('SELECT source_type FROM journal_entries WHERE id = ?')
      .get(jeId) as { source_type: string }
  ...
  it('trg_immutable_source_reference blocks UPDATE on booked entry', () => {
    ...
    expect(() => {
      db.prepare(
        "UPDATE journal_entries SET source_reference = 'tampered' WHERE id = ?",
      ).run(jeId)
  ...
  it('trg_immutable_corrects_entry_id blocks UPDATE on booked entry', () => {
    ...
    expect(() => {
      db.prepare(
        'UPDATE journal_entries SET corrects_entry_id = 999 WHERE id = ?',
      ).run(jeId)
  ...
  it('correction flow works when no payments exist (status→corrected + corrected_by_id)', () => {
    ...
    const row = db
      ...
      .get(jeId) as { status: string; corrected_by_id: number }
