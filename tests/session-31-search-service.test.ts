...
function seedBase(testDb: Database.Database) {
  ...
  const cp = createCounterparty(testDb, { name: 'Acme AB', type: 'customer', org_number: '556036-0793' })
  if (!cp.success) throw new Error('CP failed')
  const supplierCp = createCounterparty(testDb, { name: 'Leverantör AB', type: 'supplier', org_number: '556789-1234' })
  if (!supplierCp.success) throw new Error('Supplier CP failed')
  const bothCp = createCounterparty(testDb, { name: 'Rabatt 50% AB', type: 'both' })
  ...
  const accountId = testDb.prepare("SELECT id FROM accounts WHERE account_number = '3001'").get() as { id: number }
  return {
    fiscalYearId: fy.id,
    cpId: cp.data.id,
...
function bookInvoice(testDb: Database.Database, seed: ReturnType<typeof seedBase>, cpId: number, date: string) {
...
describe('globalSearch — service layer', () => {
  it('short query (<2 chars after trim) returns empty results', () => {
    ...
    const result = globalSearch(db, { query: 'A', fiscal_year_id: seed.fiscalYearId })
  ...
  it('whitespace-only query returns empty results (F15)', () => {
    const seed = seedBase(db)
    const result = globalSearch(db, { query: '   ', fiscal_year_id: seed.fiscalYearId })
    expect(getData(result).results).toHaveLength(0)
  })
  ...
  it('finds invoice by counterparty name (case-insensitive)', () => {
    ...
    const result = globalSearch(db, { query: 'Acme', fiscal_year_id: seed.fiscalYearId })
    expect(result.success).toBe(true)
  ...
  it('finds customer by name (case-insensitive)', () => {
    ...
    const result = globalSearch(db, { query: 'acme', fiscal_year_id: seed.fiscalYearId })
    expect(result.success).toBe(true)
    const customers = getData(result).results.filter(r => r.type === 'customer')
  ...
  it('supplier query returns only type IN (supplier, both) — D1', () => {
    const seed = seedBase(db)
    const result = globalSearch(db, { query: 'Leverantör', fiscal_year_id: seed.fiscalYearId })
    expect(result.success).toBe(true)
    const suppliers = getData(result).results.filter(r => r.type === 'supplier')
  ...
  it('counterparty type=both appears in BOTH customer and supplier groups — D1', () => {
    const seed = seedBase(db)
    const result = globalSearch(db, { query: 'Rabatt', fiscal_year_id: seed.fiscalYearId })
    expect(result.success).toBe(true)
    const customers = getData(result).results.filter(r => r.type === 'customer')
    const suppliers = getData(result).results.filter(r => r.type === 'supplier')
  ...
  it('finds accounts by account_number + name', () => {
    const seed = seedBase(db)
    const result = globalSearch(db, { query: '1510', fiscal_year_id: seed.fiscalYearId })
    expect(result.success).toBe(true)
  ...
  it('LIKE-escape: search "50%" searches literal percent — F8', () => {
    const seed = seedBase(db)
    const result = globalSearch(db, { query: '50%', fiscal_year_id: seed.fiscalYearId })
    expect(result.success).toBe(true)
    // "Rabatt 50% AB" should match
    const matches = getData(result).results.filter(r => r.title.includes('50%'))
  ...
  it('limit is respected', () => {
    ...
    const result = globalSearch(db, { query: '00', fiscal_year_id: seed.fiscalYearId, limit: 3 })
    expect(result.success).toBe(true)
  ...
  it('routes: invoice → /income/view/{id}, account → /account-statement?account=', () => {
    ...
    const acct = globalSearch(db, { query: '1510', fiscal_year_id: seed.fiscalYearId })
  ...
  it('case-insensitive: search "acme" matches "Acme AB"', () => {
    ...
    const result = globalSearch(db, { query: 'acme', fiscal_year_id: seed.fiscalYearId })
    expect(result.success).toBe(true)
    const customer = getData(result).results.find(r => r.type === 'customer' && r.title === 'Acme AB')
  ...
  it('expense search by description and supplier name', () => {
    ...
    const byDesc = globalSearch(db, { query: 'Kontors', fiscal_year_id: seed.fiscalYearId })
    ...
    const bySupplier = globalSearch(db, { query: 'Leverantör', fiscal_year_id: seed.fiscalYearId })
    const bySupplierExpenses = getData(bySupplier).results.filter(r => r.type === 'expense')
  ...
  it('åäö: same-case search works (Åke → Åke)', () => {
    ...
    const result = globalSearch(db, { query: 'Åke', fiscal_year_id: seed.fiscalYearId })
    const found = getData(result).results.some(r => r.title === 'Åke Andersson')
