...
describe('F46b: invoice_lines quantity CHECK', () => {
  it('rejects quantity > 9999.99 (direct SQL bypass)', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO invoice_lines (invoice_id, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, 10000, 100, 1, 1000000, 0)`).run(invoiceId)
    }).toThrow()
  })
  ...
  it('rejects quantity = 0', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO invoice_lines (invoice_id, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, 0, 100, 1, 0, 0)`).run(invoiceId)
    }).toThrow()
  })
  ...
  it('rejects quantity = -1', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO invoice_lines (invoice_id, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, -1, 100, 1, 0, 0)`).run(invoiceId)
    }).toThrow()
  })
  ...
  it('accepts quantity = 0.01 (minimum valid)', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO invoice_lines (invoice_id, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, 0.01, 100, 1, 1, 0)`).run(invoiceId)
    }).not.toThrow()
  })
  ...
  it('accepts quantity = 9999.99 (maximum valid)', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO invoice_lines (invoice_id, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, 9999.99, 100, 1, 999999, 0)`).run(invoiceId)
    }).not.toThrow()
  })
})
...
describe('F46b: expense_lines quantity CHECK', () => {
  it('rejects quantity > 9999', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO expense_lines (expense_id, account_number, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, '4010', 10000, 100, 1, 1000000, 0)`).run(expenseId)
    }).toThrow()
  })
  ...
  it('rejects quantity = 0', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO expense_lines (expense_id, account_number, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, '4010', 0, 100, 1, 0, 0)`).run(expenseId)
    }).toThrow()
  })
  ...
  it('accepts quantity = 1 (minimum valid)', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO expense_lines (expense_id, account_number, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, '4010', 1, 100, 1, 100, 0)`).run(expenseId)
    }).not.toThrow()
  })
  ...
  it('accepts quantity = 9999 (maximum valid)', () => {
    ...
    expect(() => {
      db.prepare(`INSERT INTO expense_lines (expense_id, account_number, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
        VALUES (?, '4010', 9999, 100, 1, 999900, 0)`).run(expenseId)
    }).not.toThrow()
  })
})
...
describe('F46b: migration 032 smoke', () => {
  it('preserves existing rows after recreate', () => {
    ...
    const { invoiceId } = seedMinimalData(db)
    db.prepare(`INSERT INTO invoice_lines (invoice_id, quantity, unit_price_ore, vat_code_id, line_total_ore, vat_amount_ore)
      VALUES (?, 2.5, 5000, 1, 12500, 0)`).run(invoiceId)

    const row = db.prepare('SELECT quantity FROM invoice_lines WHERE invoice_id = ?').get(invoiceId) as { quantity: number }
    expect(row.quantity).toBe(2.5)
  })

  it('idx_invoice_lines_invoice preserved after recreate', () => {
    const indexes = db
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='invoice_lines'")
      .all() as { name: string }[]
  ...
  it('trg_invoice_lines_account_number_on_finalize preserved after recreate', () => {
    const trigger = db
      .prepare("SELECT name, tbl_name FROM sqlite_master WHERE type='trigger' AND name='trg_invoice_lines_account_number_on_finalize'")
