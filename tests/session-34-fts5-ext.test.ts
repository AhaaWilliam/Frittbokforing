...
import { rebuildSearchIndex, globalSearch } from '../src/main/services/search-service'
import { createCompany } from '../src/main/services/company-service'
import { createCounterparty, updateCounterparty } from '../src/main/services/counterparty-service'
import { saveDraft, finalizeDraft, payInvoice } from '../src/main/services/invoice-service'
import {
  saveExpenseDraft,
  finalizeExpense,
  payExpense,
...
function seedBase() {
  ...
  vatCodeId = (db.prepare("SELECT id FROM vat_codes WHERE code = 'MP1' LIMIT 1").get() as { id: number }).id
  inVatCodeId = (db.prepare("SELECT id FROM vat_codes WHERE code = 'IP1' LIMIT 1").get() as { id: number }).id
}
...
describe('FTS5: invoice-sökning', () => {
  it('faktura sökbar via invoice_number efter finalize', () => {
    ...
    const inv = db.prepare("SELECT invoice_number FROM invoices WHERE status != 'draft' ORDER BY id DESC LIMIT 1")
    ...
    const data = getData(globalSearch(db, { query: inv.invoice_number, fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'invoice').length).toBeGreaterThan(0)
  })
  ...
  it('faktura sökbar via counterparty-namn', () => {
    ...
    const data = getData(globalSearch(db, { query: 'UniqueSearchCo', fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'invoice').length).toBeGreaterThan(0)
  })
  ...
  it('draft-fakturor exkluderade från FTS5-index', () => {
    ...
    saveDraft(db, {
      ...
      due_date: '2026-04-14',
      lines: [{
        product_id: null, description: 'Draft test', quantity: 1,
        unit_price_ore: 10000, vat_code_id: vatCodeId, sort_order: 0, account_number: '3002',
      }],
    })
    rebuildSearchIndex(db)

    const data = getData(globalSearch(db, { query: 'DraftExclCo', fiscal_year_id: fyId }))
  ...
  it('ny finaliserad faktura sökbar direkt (rebuild efter finalize)', () => {
    ...
    const data = getData(globalSearch(db, { query: 'PostFinalizeCo', fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'invoice').length).toBeGreaterThan(0)
  })
})
...
describe('FTS5: expense-sökning', () => {
  it('expense sökbar via description', () => {
    ...
    const data = getData(globalSearch(db, { query: 'Testkostnad', fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'expense').length).toBeGreaterThan(0)
  })
  ...
  it('expense sökbar via counterparty-namn', () => {
    ...
    const data = getData(globalSearch(db, { query: 'UniqueSupplierCo', fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'expense').length).toBeGreaterThan(0)
  })
  ...
  it('draft-expenses exkluderade', () => {
    ...
    saveExpenseDraft(db, {
      ...
      lines: [{ description: 'Test', account_number: '6110', quantity: 1, unit_price_ore: 10000, vat_code_id: inVatCodeId }],
    })
    rebuildSearchIndex(db)

    const data = getData(globalSearch(db, { query: 'DraftExpenseCo', fiscal_year_id: fyId }))
...
describe('FTS5: betalning + namnändring', () => {
  it('betalning → fortfarande sökbar efter rebuild', () => {
    ...
    const inv = db.prepare('SELECT total_amount_ore FROM invoices WHERE id = ?').get(invoiceId) as { total_amount_ore: number }
    payInvoice(db, {
    ...
    const data = getData(globalSearch(db, { query: 'PayTestCo', fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'invoice').length).toBeGreaterThan(0)
  })
  ...
  it('counterparty-namnändring → faktura sökbar via nytt namn', () => {
    ...
    const data = getData(globalSearch(db, { query: 'NewNameCo', fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'invoice').length).toBeGreaterThan(0)
  })
})
...
describe('FTS5: LIKE fallback', () => {
  it('invoice-sök fungerar utan search_index-tabell', () => {
    ...
    const data = getData(globalSearch(db, { query: 'FallbackCo', fiscal_year_id: fyId }))
    expect(data.results.filter(r => r.type === 'invoice').length).toBeGreaterThan(0)
  })
})
