...
describe('lower_unicode function (F58)', () => {
  it.each([
    ...
    const row = db.prepare('SELECT lower_unicode(?) AS r').get(input) as { r: string }
    expect(row.r).toBe(expected)
  })

  it('NULL passthrough', () => {
    const row = db.prepare('SELECT lower_unicode(NULL) AS r').get() as { r: null }
    expect(row.r).toBeNull()
  })

  it('non-string passthrough (integer)', () => {
    const row = db.prepare('SELECT lower_unicode(42) AS r').get() as { r: number }
    expect(row.r).toBe(42)
  })

  it('deterministic sanity', () => {
    const row = db.prepare("SELECT lower_unicode('Å') AS r").get() as { r: string }
    expect(row.r).toBe('å')
  })
  ...
  it('known limitation: eszett folding not supported (FTS5 scope)', () => {
    ...
    const r1 = db.prepare("SELECT lower_unicode('STRASSE') AS r").get() as { r: string }
    const r2 = db.prepare("SELECT lower_unicode('Straße') AS r").get() as { r: string }
...
describe('cross-case search (service level, F58)', () => {
  it('"åke" matches "Åke Andersson"', () => {
    ...
    const found = getData(globalSearch(db, { query: 'åke', fiscal_year_id: fyId }))
  ...
  it('"östgöta" matches "Östgöta Bygg AB"', () => {
    ...
    const found = getData(globalSearch(db, { query: 'östgöta', fiscal_year_id: fyId }))
  ...
  it('"ACME" matches "Acme AB" (ASCII case regression)', () => {
    ...
    const found = getData(globalSearch(db, { query: 'ACME', fiscal_year_id: fyId }))
...
describe('F8 regression after lower_unicode migration', () => {
  it('literal % is not a wildcard', () => {
    ...
    const results = getData(globalSearch(db, { query: '50%', fiscal_year_id: fyId })).results
  ...
  it('literal _ is not a wildcard (FTS5: _ is token separator, "50_" matches "50" prefix)', () => {
    ...
    const results = getData(globalSearch(db, { query: '50_', fiscal_year_id: fyId })).results
    expect(results.some(r => r.title === 'Rabatt 50% AB')).toBe(true)
  })
})
