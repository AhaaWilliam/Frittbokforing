...
function ftsSearch(db: Database.Database, entityType: FtsEntityType, query: string, limit: number, fiscalYearId?: number): number[] | null {
  try {
    ...
    const rows = db.prepare('SELECT entity_id FROM search_index WHERE search_index MATCH ? LIMIT ?')
      .all(matchExpr, limit) as Array<{ entity_id: number }>
    return rows.map(r => Number(r.entity_id))
  } catch { return null }
}

function ftsOrFallback<T>(db: Database.Database, entityType: FtsEntityType, trimmed: string, limit: number, fiscalYearId: number | undefined, likeFn: () => T[], ftsFn: (ids: number[]) => T[]): T[] {
...
export function globalSearch(db: Database.Database, input: SearchInput): IpcResult<GlobalSearchResponse> {
  const trimmed = input.query.trim()
  if (trimmed.length < 2) return { success: true, data: { results: [], total_count: 0 } }
  ...
  {
    const idList = invoiceFtsIds !== null && invoiceFtsIds.length > 0 ? invoiceFtsIds.join(',') : '-1'
    ...
    `).all({ fy, pattern, lim: perLimit }) as Array<{ id: number; invoice_number: string; total_amount_ore: number; status: string; cp_name: string }>
    for (const r of invoiceRows) {
      results.push({ type: 'invoice', identifier: String(r.id), title: `#${r.invoice_number} — ${r.cp_name}`, subtitle: `${formatOreKr(r.total_amount_ore)} · ${STATUS_LABELS[r.status] ?? r.status}`, route: `/income/view/${r.id}` })
    }
  }
  ...
  {
    const idList = expenseFtsIds !== null && expenseFtsIds.length > 0 ? expenseFtsIds.join(',') : '-1'
    ...
    `).all({ fy, pattern, lim: perLimit }) as Array<{ id: number; supplier_invoice_number: string | null; description: string; total_amount_ore: number; status: string; cp_name: string }>
    for (const r of expenseRows) {
      const title = r.supplier_invoice_number ? `${r.supplier_invoice_number} — ${r.cp_name}` : `${r.description} — ${r.cp_name}`
      results.push({ type: 'expense', identifier: String(r.id), title, subtitle: `${formatOreKr(r.total_amount_ore)} · ${STATUS_LABELS[r.status] ?? r.status}`, route: `/expenses/view/${r.id}` })
    }
  }
  ...
  if (customerIds !== null && customerIds.length > 0) {
    ...
    const rows = db.prepare(`SELECT id, name, org_number FROM counterparties WHERE id IN (${ph}) AND type IN ('customer','both') AND is_active=1 ORDER BY name ASC LIMIT ?`).all(...customerIds, perLimit) as Array<{ id: number; name: string; org_number: string | null }>
    for (const r of rows) results.push({ type: 'customer', identifier: String(r.id), title: r.name, subtitle: `Kund${r.org_number ? ` · ${r.org_number}` : ''}`, route: `/customers/${r.id}` })
  } else if (customerIds === null) {
    const rows = db.prepare(`SELECT id, name, org_number FROM counterparties WHERE type IN ('customer','both') AND is_active=1 AND (lower_unicode(name) LIKE lower_unicode(:pattern) ESCAPE '!' OR lower_unicode(org_number) LIKE lower_unicode(:pattern) ESCAPE '!') ORDER BY name ASC LIMIT :lim`).all({ pattern, lim: perLimit }) as Array<{ id: number; name: string; org_number: string | null }>
    for (const r of rows) results.push({ type: 'customer', identifier: String(r.id), title: r.name, subtitle: `Kund${r.org_number ? ` · ${r.org_number}` : ''}`, route: `/customers/${r.id}` })
  }
  ...
  if (supplierIds !== null && supplierIds.length > 0) {
    ...
    const rows = db.prepare(`SELECT id, name, org_number FROM counterparties WHERE id IN (${ph}) AND type IN ('supplier','both') AND is_active=1 ORDER BY name ASC LIMIT ?`).all(...supplierIds, perLimit) as Array<{ id: number; name: string; org_number: string | null }>
    for (const r of rows) results.push({ type: 'supplier', identifier: String(r.id), title: r.name, subtitle: `Leverantör${r.org_number ? ` · ${r.org_number}` : ''}`, route: `/suppliers/${r.id}` })
  } else if (supplierIds === null) {
    const rows = db.prepare(`SELECT id, name, org_number FROM counterparties WHERE type IN ('supplier','both') AND is_active=1 AND (lower_unicode(name) LIKE lower_unicode(:pattern) ESCAPE '!' OR lower_unicode(org_number) LIKE lower_unicode(:pattern) ESCAPE '!') ORDER BY name ASC LIMIT :lim`).all({ pattern, lim: perLimit }) as Array<{ id: number; name: string; org_number: string | null }>
    for (const r of rows) results.push({ type: 'supplier', identifier: String(r.id), title: r.name, subtitle: `Leverantör${r.org_number ? ` · ${r.org_number}` : ''}`, route: `/suppliers/${r.id}` })
  }
  ...
  if (productIds !== null && productIds.length > 0) {
    ...
    const rows = db.prepare(`SELECT id, name, default_price_ore, unit FROM products WHERE id IN (${ph}) AND is_active=1 ORDER BY name ASC LIMIT ?`).all(...productIds, perLimit) as Array<{ id: number; name: string; default_price_ore: number; unit: string }>
    for (const r of rows) results.push({ type: 'product', identifier: String(r.id), title: r.name, subtitle: `${formatOreKr(r.default_price_ore)}/${r.unit}`, route: `/products/${r.id}` })
  } else if (productIds === null) {
    const rows = db.prepare(`SELECT id, name, default_price_ore, unit FROM products WHERE is_active=1 AND lower_unicode(name) LIKE lower_unicode(:pattern) ESCAPE '!' ORDER BY name ASC LIMIT :lim`).all({ pattern, lim: perLimit }) as Array<{ id: number; name: string; default_price_ore: number; unit: string }>
    for (const r of rows) results.push({ type: 'product', identifier: String(r.id), title: r.name, subtitle: `${formatOreKr(r.default_price_ore)}/${r.unit}`, route: `/products/${r.id}` })
  }
  ...
  if (accountIds !== null && accountIds.length > 0) {
    ...
    const rows = db.prepare(`SELECT account_number, name FROM accounts WHERE id IN (${ph}) AND is_active=1 ORDER BY account_number ASC LIMIT ?`).all(...accountIds, perLimit) as Array<{ account_number: string; name: string }>
    for (const r of rows) { const cls = parseInt(r.account_number[0]); results.push({ type: 'account', identifier: r.account_number, title: `${r.account_number} ${r.name}`, subtitle: `Klass ${cls} — ${ACCOUNT_CLASS_NAMES[cls] ?? ''}`, route: `/account-statement?account=${r.account_number}` }) }
  } else if (accountIds === null) {
    const rows = db.prepare(`SELECT account_number, name FROM accounts WHERE is_active=1 AND (lower_unicode(account_number) LIKE lower_unicode(:pattern) ESCAPE '!' OR lower_unicode(name) LIKE lower_unicode(:pattern) ESCAPE '!') ORDER BY account_number ASC LIMIT :lim`).all({ pattern, lim: perLimit }) as Array<{ account_number: string; name: string }>
    for (const r of rows) { const cls = parseInt(r.account_number[0]); results.push({ type: 'account', identifier: r.account_number, title: `${r.account_number} ${r.name}`, subtitle: `Klass ${cls} — ${ACCOUNT_CLASS_NAMES[cls] ?? ''}`, route: `/account-statement?account=${r.account_number}` }) }
  }
  ...
  `).all({ fy, pattern, lim: perLimit, is_ver_ref: isVerRef, ver_series: verSeries, ver_num: verNum }) as Array<{
    id: number; verification_number: number; verification_series: string; description: string; journal_date: string; status: string
    corrects_entry_id: number | null; corrected_by_id: number | null; manual_entry_id: number | null; original_manual_entry_id: number | null
  }>
  ...
  for (const r of journalRows) {
    ...
    const route = r.corrects_entry_id !== null ? `/manual-entries/view/${targetMeId}?highlight=${ref}` : `/manual-entries/view/${targetMeId}`
    results.push({ type: 'journal_entry', identifier: String(r.id), title, subtitle, route })
  }

  return { success: true, data: { results: results.slice(0, perLimit), total_count: results.length } }
}
